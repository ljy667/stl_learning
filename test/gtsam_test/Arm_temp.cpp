...
void Arm::forwardKinematics(
    ...
    std::optional<std::reference_wrapper<std::vector<Matrix>>> J_jpx_jp,
    std::optional<std::reference_wrapper<std::vector<Matrix>>> J_jpv_jp,
    std::optional<std::reference_wrapper<std::vector<Matrix>>> J_jvx_jp) const {
  ...
  if (J_jpx_jp) J_jpx_jp->get().assign(dof(), Matrix::Zero(6, dof()));
  if (J_jvx_jp) J_jvx_jp->get().assign(dof(), Matrix::Zero(3, dof()));
  if (J_jpv_jp) J_jpv_jp->get().assign(dof(), Matrix::Zero(3, dof()));
  ...
  for (size_t i = 0; i < dof(); i++) {
    ...
    if (jv && jvx)
      jvx->get()[i] = J[i] * jv->get();

    // diff Ho[i] = Ho[i-1] * H[i] to get pJp
    if (J_jpx_jp) {
      Matrix& Jp = J_jpx_jp->get()[i];
